...
def exercise_controls() -> dict[str, int]:
    ...
    receipt = cache.invalidate([digest], (), (), prior_epoch=0, new_epoch=1)
    if digest not in receipt["invalidated_keys"]:
    ...
    return {"controls": len(CONTROL_NAMES), "omission_mutations": len(CACHE_KEY_FIELDS), "near_miss_refusals": misses, "invalidation_receipts": 1}
