...
def apply_json_patch(instance: Any, operations: list[dict[str, Any]]) -> Any:
    """Apply the closed RFC-6902 subset used by the frozen fixture registry."""

    value = deepcopy(instance)
    for operation in operations:
        operation_name = operation.get("op")
        if operation_name not in {"add", "remove", "replace"} or not isinstance(operation.get("path"), str):
        ...
        if not tokens:
            if operation_name == "remove":
                raise Refusal("root removal is forbidden")
            if "value" not in operation:
                raise Refusal("patch value missing")
        ...
        if isinstance(target, list):
            if operation_name == "add":
                if last == "-":
                    ...
                    target.insert(int(last), deepcopy(operation["value"]))
            ...
            if last not in target:
                raise Refusal("patch removal target missing")
            del target[last]
        else:
            if operation_name == "replace" and last not in target:
                raise Refusal("patch replacement target missing")
            if "value" not in operation:
                raise Refusal("patch value missing")
