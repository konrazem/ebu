...
IDENTITY_KINDS = {
    "binding_implementation_identity": "stage_f_binding_implementation/v3",
...
CONTROL_RECORD_INDEX_KEYS = frozenset({"schema", "entries", "entry_count"})
CONTROL_RECORD_ENTRY_KEYS = frozenset({"definition", "path"})
...
def _load_authority_schema(path: Path, raw: bytes | None = None) -> object:
    ...
    def count_local_references(value: object) -> int:
                ...
                if key == "$ref":
                    if not isinstance(item, str) or not item.startswith("#/$defs/"):
                        raise BindingRefusal(
                            "binding evidence schema contains a nonlocal reference"
                        )
                    count += 1
                count += count_local_references(item)
            return count
        if isinstance(value, list):
            return sum(count_local_references(item) for item in value)
        return 0
    ...
    if (
        ...
        or count_local_references(schema) != AUTHORITY_SCHEMA_LOCAL_REFERENCE_COUNT
    ):
        raise BindingRefusal("binding evidence schema counts or roots differ")
    return schema
...
def validate_source_artifacts(
    ...
    *,
    ...
    if (
        ...
        != {
            "kind": "stage_f_binding_authority_set/v3",
    ...
    if (
        authority_commit != FINAL_AUTHORITY_INTEGRATION_COMMIT
        or repository.tree_id(ACCEPTED_BASE["commit"]) != ACCEPTED_BASE["tree"]
        ...
        != FINAL_REACHABILITY_INTEGRATION_TREE
        or repository.commit(CORRECTION_AUTHORITY_INTEGRATION_COMMIT)[1]
        != (
            CORRECTION_REQUIRED_TARGET_COMMIT,
            CORRECTION_AUTHORITY_CANDIDATE_COMMIT,
        )
        or repository.commit(ORIGINAL_AUTHORITY_INTEGRATION_COMMIT)[1]
        != (ACCEPTED_BASE["commit"], ORIGINAL_AUTHORITY_CANDIDATE_COMMIT)
        or repository.commit(REACHABILITY_CANDIDATE_COMMIT)[1]
        != (CORRECTION_AUTHORITY_INTEGRATION_COMMIT,)
        or repository.commit(REACHABILITY_INTEGRATION_COMMIT)[1]
        != (
            CORRECTION_AUTHORITY_INTEGRATION_COMMIT,
            REACHABILITY_CANDIDATE_COMMIT,
        )
        ...
        or not repository.is_ancestor(
            FINAL_REACHABILITY_INTEGRATION_COMMIT, implementation_commit
        )
        or any(
    ...
    if (
        authority_preimage["local_authority_file_count"] != 18
        or len(authority_preimage["ordered_local_authority_file_rows"]) != 18
        or binding_preimage["integrated_authority_file_count"] != 18
        or len(binding_preimage["ordered_integrated_authority_file_rows"]) != 18
        or binding_preimage["implementation_file_count"] != 14
        or len(binding_preimage["ordered_implementation_file_rows"]) != 14
    ):
        raise BindingRefusal("v3 authority or implementation row count differs")
    diff_rows = repository.diff_status(
        FINAL_REACHABILITY_INTEGRATION_COMMIT, implementation_commit
    )
    expected_status = {
    ...
    stage_e_manifest_row = repository.row(
        implementation_commit,
        STAGE_E_PATH_MANIFEST,
        allowed_modes=frozenset({"100644"}),
    )
    stage_e_manifest = strict_loads(
        repository.blob(stage_e_manifest_row["git_object"])
    )
    stage_e_scope = stage_e_manifest["prospective_harness_implementation"]
    stage_e_paths = tuple(stage_e_scope["modified_paths"]) + tuple(
        stage_e_scope["new_paths"]
    )
    ...
    if (
        stage_e_scope["modified_path_count"] != 1
        or stage_e_scope["new_path_count"] != 50
        or stage_e_scope["total_path_count"] != 51
        or len(stage_e_paths) != 51
        or len(set(stage_e_paths)) != 51
        or len(expected_descendant_paths) != 82
    ...
    for ordinal, (expected_path, row) in enumerate(zip(
        AUTHORITY_PATHS,
        authority_preimage["ordered_local_authority_file_rows"],
        strict=True,
    )):
        ...
        if row != repository.row(
            ...
            candidate_commit, expected_path, allowed_modes=frozenset({"100644"})
        ):
    ...
    return {
        "schema": "stage_f_binding_foundation_artifact_validation/v3",
...
def _load_control_records(index_path: Path, read_material) -> list[dict[str, object]]:
    """Load the exact ordered v2 control chain from retained canonical files."""

    index = _closed_keys(
        strict_loads(
            read_material(index_path, "Stage F control-record index"),
            require_canonical=True,
        ),
        CONTROL_RECORD_INDEX_KEYS,
        "Stage F control-record index",
    )
    entries = index["entries"]
    if (
        index["schema"] != "stage_f_control_record_material_index/v1"
        or type(entries) is not list
        or not entries
        or type(index["entry_count"]) is not int
        or isinstance(index["entry_count"], bool)
        ...
        raise BindingRefusal("Stage F control-record index shape or count differs")
    records: list[dict[str, object]] = []
    paths: set[str] = set()
    for ordinal, untrusted in enumerate(entries):
        entry = _closed_keys(
            untrusted,
            CONTROL_RECORD_ENTRY_KEYS,
            f"Stage F control-record entry {ordinal}",
        )
        definition = entry["definition"]
        path_text = entry["path"]
        if (
            type(definition) is not str
            or re.fullmatch(r"[a-z][a-z0-9_]*", definition) is None
            or type(path_text) is not str
            or not Path(path_text).is_absolute()
            or path_text in paths
        ):
            raise BindingRefusal(
                f"Stage F control-record path {ordinal} is invalid or repeated"
            )
        paths.add(path_text)
        record = strict_loads(
            read_material(Path(path_text), f"Stage F control record {ordinal}"),
            require_canonical=True,
        )
        if type(record) is not dict:
            raise BindingRefusal(f"Stage F control record {ordinal} is not an object")
        records.append({"definition": definition, "record": record})
    return records
...
def _validate_record_chain_locked(
    ...
    result: dict[str, object] = {
        "schema": (
            "stage_f_prestart_controller_authorization_validation/v2"
            if launch_gate
            else "stage_f_binding_readiness_chain_validation/v2"
        ),
        "disposition": (
            "POST_PACKET_AUTHORIZATION_VALIDATED_PRESTART_CONTROLLER_REQUIRED"
            if launch_gate
...
def validate_control_state(args: argparse.Namespace) -> dict[str, object]:
    """Validate retained v2 control evidence without issuing a control call."""

    if _BOOTSTRAP_LOCK_CONTEXT is None:
        raise BindingRefusal("control-state validation requires the locked validator")
    retained_root = os.fspath(args.retained_root)
    with _RetainedMaterialLocks(retained_root) as locks:
        schema = _load_authority_schema(
            args.schema,
            locks.read(args.schema, "retained effective authority schema"),
        )
        validator = ClosedSchemaValidator(schema)
        rows = _load_control_records(args.control_record_index, locks.read)
        summary = validate_stage_f_control_state_machine(
            rows,
            lambda definition, record: validate_corrected_record(
                validator, definition, record, as_root=True
            ),
        )
    if not isinstance(summary, Mapping):
        raise BindingRefusal("control-state validator returned a non-object")
    summary_counters = summary.get("scientific_counters")
    assert_zero_science_counters(summary_counters)
    result = {
        key: value for key, value in summary.items() if key != "scientific_counters"
    }
    result.update(ZERO_SCIENCE_COUNTERS)
    return result
...
def _parser() -> argparse.ArgumentParser:
    ...
    artifacts = subparsers.add_parser(
        "source-artifacts", add_help=False, allow_abbrev=False
    )
    ...
    control_state = subparsers.add_parser(
        "control-state", add_help=False, allow_abbrev=False
    )
    control_state.add_argument("--schema", type=Path, required=True)
    control_state.add_argument("--retained-root", type=Path, required=True)
    control_state.add_argument("--control-record-index", type=Path, required=True)
    durability = subparsers.add_parser(
        "durability-receipt", add_help=False, allow_abbrev=False
    )
    ...
    for command, full_authorization_required in (
        ...
        chain = subparsers.add_parser(
            command, add_help=False, allow_abbrev=False
        )
        for name in (
            "schema",
            "private-manifest",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if arguments and arguments[0] == "--validator-zipapp":
            ...
            result = validate_control_state(args)
