...
def _load_identity_projection(path: Path) -> dict[str, dict[str, str]]:
    ...
    if (
        ...
        or value.get("schema") != "stage_f_binding_validator_identity_projection/v1"
    ):
    ...
    def __init__(self, source: Path) -> None:
        ...
        if os.path.lexists(pack_directory):
            ...
            for path in entries:
                ...
                if PACK_FILE_RE.fullmatch(path.name) is None:
...
def _parse_git_batch(
    ...
    for requested_id in ordered_object_ids:
        ...
        if hashlib.sha1(framed, usedforsecurity=False).hexdigest() != requested_id:
            raise BuildRefusal(f"reachable Git object content address differs: {requested_id}")
...
def build(
    ...
    implementation_preimage = {
        "schema": "stage_f_binding_foundation_build_implementation_projection/v1",
    ...
    if identity_projection_path is None:
        identities = {
            "binding_implementation_identity": _identity(
                "stage_f_binding_implementation/v1", implementation_preimage
            ),
