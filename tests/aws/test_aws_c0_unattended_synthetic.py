...
class CompletePreparationBuilderTests(unittest.TestCase):
    def sealed_source_fields(self):
        ...
        compatibility=json.loads((ROOT/'aws_c0_sealed_source_downstream_carrier_compatibility_contract.json').read_bytes())
        runtime=json.loads((ROOT/'aws_c0_runtime_artifact_compatibility_successor_contract.json').read_bytes())
        ...
        return {
            ...
            'sealed_source_downstream_carrier_compatibility_contract_identity':G.identity(
                compatibility['schema'],compatibility),
            'runtime_artifact_compatibility_successor_contract_identity':G.identity(
    ...
    def test_schema_upgrade_is_narrow_and_uses_exact_existing_lineage(self):
        ...
        self.assertEqual(p['artifact_source_mode']['const'],
                         'EXACT_SIX_SEALED_VERSION_REUSE_PLUS_TWO_RUNTIME_SUCCESSORS')
        ...
        self.assertEqual(p['fresh_new_pre_live_record_count']['const'], 18)
        self.assertEqual(p['new_artifact_put_count']['const'], 2)
        ...
        self.assertEqual(set(schema['allOf'][1]['required']),
            ...
            {'artifact_source_mode','sealed_source_transfer_proposal_identity',
             ...
             'sealed_source_downstream_carrier_compatibility_contract_identity',
             'runtime_artifact_compatibility_successor_contract_identity'})

    def test_transitive_successors_preserve_historical_definitions(self):
        definitions=json.loads((ROOT/'aws_c0_deployment_sequence_evidence_schema.json').read_bytes())['$defs']
        old_launch=definitions['launch_v7_definition']['allOf'][1]['properties']
        new_launch=definitions['launch_v8_definition']['allOf'][1]['properties']
        self.assertEqual(old_launch['schema']['const'],'aws_c0_launch_request/v7')
        self.assertEqual(old_launch['preparation_packet_identity']['allOf'][1]['properties']['kind']['const'],
                         'aws_c0_preparation_packet/v6')
        self.assertEqual(new_launch['schema']['const'],'aws_c0_launch_request/v8')
        self.assertEqual(new_launch['preparation_packet_identity']['allOf'][1]['properties']['kind']['const'],
                         'aws_c0_preparation_packet/v8')
        self.assertEqual(new_launch['preparation_authorization_identity']['allOf'][1]['properties']['kind']['const'],
                         'aws_c0_preparation_authorization/v7')
        self.assertEqual(definitions['preparation_closure_v7_definition']['allOf'][1]['properties'][
            'preparation_packet_identity']['allOf'][1]['properties']['kind']['const'],
            'aws_c0_preparation_packet/v8')
        self.assertEqual(definitions['live_packet_v8_definition']['allOf'][1]['properties'][
            'preparation_closure_identity']['allOf'][1]['properties']['kind']['const'],
            'aws_c0_preparation_closure/v7')
        self.assertEqual(definitions['live_authorization_v8_definition']['allOf'][1]['properties'][
            'live_packet_identity']['allOf'][1]['properties']['kind']['const'],
            'aws_c0_live_packet/v8')
        newest_launch=definitions['launch_v9_definition']['allOf'][1]['properties']
        self.assertEqual(newest_launch['schema']['const'],'aws_c0_launch_request/v9')
        self.assertEqual(newest_launch['preparation_packet_identity']['allOf'][1]['properties']['kind']['const'],
                         'aws_c0_preparation_packet/v9')
        self.assertEqual(newest_launch['preparation_authorization_identity']['allOf'][1]['properties']['kind']['const'],
                         'aws_c0_preparation_authorization/v8')
        self.assertEqual(definitions['preparation_closure_v8_definition']['allOf'][1]['properties'][
            'preparation_packet_identity']['allOf'][1]['properties']['kind']['const'],
            'aws_c0_preparation_packet/v9')
        self.assertEqual(definitions['live_packet_v9_definition']['allOf'][1]['properties'][
            'preparation_closure_identity']['allOf'][1]['properties']['kind']['const'],
            'aws_c0_preparation_closure/v8')
        self.assertEqual(definitions['live_authorization_v9_definition']['allOf'][1]['properties'][
            'live_packet_identity']['allOf'][1]['properties']['kind']['const'],
            'aws_c0_live_packet/v9')
    ...
    def test_sealed_source_packet_fields_bind_exact_existing_versions(self):
        ...
        for mutation,pattern in (
            (lambda x:x['sealed_artifact_version_receipts'][0].update(version_id='wrong'),'six unchanged'),
...
class ReconstructionV4IntegrationTests(unittest.TestCase):
    def material(self):
        validation,sealed,_,bundles,outputs,plan_fields=ReconstructionV3IntegrationTests().material()
        bucket_receipts,bucket_context,_,_=BucketControlsKmsReconstructionTests().material()
        bucket=G.build_bucket_controls_kms_output(ROOT,bucket_receipts,**bucket_context)
        bucket_bundle={'candidate':bucket,'receipts':bucket_receipts,'context':bucket_context}
        sealed=copy.deepcopy(sealed);sealed['schema']='aws_c0_predeployment_reconstruction_context/v3'
        sealed['expected_bucket_name']=bucket_context['expected_bucket_name']
        sealed['expected_bucket_identity']=bucket_context['expected_bucket_identity']
        sealed_id=F.identity(sealed['schema'],F.digest(F.canonical_bytes(sealed)))
        account,profile,iam,vpc,quota=bundles
        outputs={**outputs,'BUCKET_CONTROLS_KMS':bucket}
        return validation,sealed,sealed_id,(account,profile,iam,bucket_bundle,vpc,quota),outputs,plan_fields

    def test_progress_v4_binds_six_candidates_and_leaves_five_unresolved(self):
        validation,_,_,_,outputs,plan_fields=self.material()
        record=G.build_runtime_control_reconstruction_progress_v4(ROOT,outputs,
        ...
        self.assertEqual(len(record['unresolved_control_ids_in_order']),5)
        self.assertEqual(F.validate_runtime_control_reconstruction_progress_v4(record,
        ...
        G.validate_record(ROOT,'runtime_reconstruction_progress_v4',record)

    def test_attachment_v3_reruns_six_and_binds_exact_v2_predecessor(self):
        validation,sealed,sealed_id,bundles,_,plan_fields=self.material()
        account,profile,iam,bucket,vpc,quota=bundles
        record=G.build_runtime_control_reconstruction_source_attachment_v3(ROOT,
        ...
        self.assertEqual(record['previous_source_attachment_identity']['kind'],
            'aws_c0_runtime_control_reconstruction_source_attachment/v2')
        self.assertEqual(F.validate_runtime_control_reconstruction_source_attachment_v3(record,
            read_plan=plan_fields['runtime_control_read_plan'],
        ...
        G.validate_record(ROOT,'runtime_reconstruction_source_attachment_v3',record)

    def test_attachment_v3_refuses_bucket_context_candidate_or_predecessor_substitution(self):
        for mutate in ('context','candidate'):
            validation,sealed,sealed_id,bundles,_,_=self.material()
            account,profile,iam,bucket,vpc,quota=copy.deepcopy(bundles)
            if mutate=='context':bucket['context']['expected_bucket_name']='substituted-bucket'
            else:bucket['candidate']['decoded_json']['bucket_policy_sha256']='f'*64
            with self.subTest(mutate=mutate),self.assertRaisesRegex(Exception,
                    'bucket/KMS source (bundle context|attachment candidate)'):
                G.build_runtime_control_reconstruction_source_attachment_v3(ROOT,
                    account_bundle=account,instance_profile_bundle=profile,iam_bundle=iam,
                    bucket_controls_kms_bundle=bucket,vpc_bundle=vpc,service_quota_bundle=quota,
                    sealed_context=sealed,sealed_context_identity=sealed_id,validation_utc=validation)
        validation,sealed,sealed_id,bundles,_,plan_fields=self.material()
        ...
        record=G.build_runtime_control_reconstruction_source_attachment_v3(ROOT,
            ...
            bucket_controls_kms_bundle=bucket,vpc_bundle=vpc,service_quota_bundle=quota,
            sealed_context=sealed,sealed_context_identity=sealed_id,validation_utc=validation)
        record['previous_source_attachment_identity']['sha256']='f'*64
        with self.assertRaises(F.Refusal):
            F.validate_runtime_control_reconstruction_source_attachment_v3(record,
                read_plan=plan_fields['runtime_control_read_plan'],
                read_plan_identity=plan_fields['runtime_control_read_plan_identity'],phase='PREDEPLOYMENT',
...
class ReconstructionV6IntegrationTests(unittest.TestCase):
    def material(self):
        validation,sealed,_,bundles,outputs,plan_fields=ReconstructionV5IntegrationTests().material()
        change_receipts,change_context=ChangeSetEffectsReconstructionTests().material()
        change_context['validation_utc']=validation
        change=G.build_change_set_effects_output(ROOT,change_receipts,**change_context)
        change_bundle={'candidate':change,'receipts':change_receipts,'context':change_context}
        sealed=copy.deepcopy(sealed);sealed['schema']='aws_c0_predeployment_reconstruction_context/v5'
        for field in ('expected_change_set_arn','expected_stack_name','expected_template_sha256',
                'expected_effect_actions_in_order','cfn_pagination_bounds'):
            sealed[field]=copy.deepcopy(change_context[field])
        sealed_id=F.identity(sealed['schema'],F.digest(F.canonical_bytes(sealed)))
        outputs={**outputs,'CHANGE_SET_AND_EFFECTS':change}
        account,profile,iam,bucket,vpc,quota,artifact=bundles
        return validation,sealed,sealed_id,(account,profile,iam,bucket,vpc,quota,change_bundle,artifact),outputs,plan_fields

    def test_progress_v6_binds_eight_candidates_and_leaves_three_unresolved(self):
        validation,_,_,_,outputs,plan_fields=self.material()
        record=G.build_runtime_control_reconstruction_progress_v6(ROOT,outputs,
            phase='PREDEPLOYMENT',validation_utc=validation)
        self.assertEqual(record['candidate_control_ids_in_order'],['ACCOUNT_REGION',
            'INSTANCE_PROFILE_SOLE_ROLE','IAM_POLICY_SET','BUCKET_CONTROLS_KMS',
            'VPC_NETWORK_PATH','SERVICE_QUOTA','CHANGE_SET_AND_EFFECTS','ARTIFACT_VERSION_SET'])
        self.assertEqual(record['unresolved_control_ids_in_order'],
            ['STANDARD_WORKFLOW','SSM_DOCUMENT','SOFTWARE_AND_IMAGE_SET'])
        self.assertEqual(F.validate_runtime_control_reconstruction_progress_v6(record,
            read_plan=plan_fields['runtime_control_read_plan'],
            read_plan_identity=plan_fields['runtime_control_read_plan_identity'],
            phase='PREDEPLOYMENT',validation_utc=validation),record)
        G.validate_record(ROOT,'runtime_reconstruction_progress_v6',record)

    def test_attachment_v5_reruns_eight_and_binds_exact_v4_predecessor(self):
        validation,sealed,sealed_id,bundles,_,plan_fields=self.material()
        account,profile,iam,bucket,vpc,quota,change,artifact=bundles
        record=G.build_runtime_control_reconstruction_source_attachment_v5(ROOT,
            account_bundle=account,instance_profile_bundle=profile,iam_bundle=iam,
            bucket_controls_kms_bundle=bucket,vpc_bundle=vpc,service_quota_bundle=quota,
        ...
        self.assertEqual([item['control'] for item in record['outputs_in_order']],
            ['ACCOUNT_REGION','INSTANCE_PROFILE_SOLE_ROLE','IAM_POLICY_SET','BUCKET_CONTROLS_KMS',
                'VPC_NETWORK_PATH','SERVICE_QUOTA','CHANGE_SET_AND_EFFECTS','ARTIFACT_VERSION_SET'])
        self.assertEqual(record['previous_source_attachment_identity']['kind'],
            'aws_c0_runtime_control_reconstruction_source_attachment/v4')
        self.assertEqual(F.validate_runtime_control_reconstruction_source_attachment_v5(record,
            read_plan=plan_fields['runtime_control_read_plan'],
            read_plan_identity=plan_fields['runtime_control_read_plan_identity'],phase='PREDEPLOYMENT',
            validation_utc=validation,expected_sealed_context_identity=sealed_id),record)
        G.validate_record(ROOT,'runtime_reconstruction_source_attachment_v5',record)

    def test_attachment_v5_refuses_change_context_candidate_or_predecessor_substitution(self):
        for mutate in ('context','candidate'):
            validation,sealed,sealed_id,bundles,_,_=self.material()
            account,profile,iam,bucket,vpc,quota,change,artifact=copy.deepcopy(bundles)
            if mutate=='context':change['context']['expected_stack_name']='Other'
            else:change['candidate']['decoded_json']['change_set_status']='CREATE_FAILED'
            with self.subTest(mutate=mutate),self.assertRaisesRegex(Exception,
                    'change-set source (bundle context|attachment candidate)'):
                G.build_runtime_control_reconstruction_source_attachment_v5(ROOT,
                    account_bundle=account,instance_profile_bundle=profile,iam_bundle=iam,
                    bucket_controls_kms_bundle=bucket,vpc_bundle=vpc,service_quota_bundle=quota,
                    change_set_effects_bundle=change,artifact_version_set_bundle=artifact,
                    sealed_context=sealed,sealed_context_identity=sealed_id,validation_utc=validation)
        ...
        record=G.build_runtime_control_reconstruction_source_attachment_v5(ROOT,
            ...
            change_set_effects_bundle=change,artifact_version_set_bundle=artifact,
            sealed_context=sealed,sealed_context_identity=sealed_id,validation_utc=validation)
        record['previous_source_attachment_identity']['sha256']='0'*64
        with self.assertRaises(F.Refusal):
            F.validate_runtime_control_reconstruction_source_attachment_v5(record,
                read_plan=plan_fields['runtime_control_read_plan'],
                read_plan_identity=plan_fields['runtime_control_read_plan_identity'],phase='PREDEPLOYMENT',
                validation_utc=validation,expected_sealed_context_identity=sealed_id)
...
class ServiceQuotaReconstructionTests(unittest.TestCase):
    def material(self):
        ...
        for index,row in enumerate(('R34','R35')):
            receipt=copy.deepcopy(sources[0]);request_id='offline-quota-'+row
            receipt.update(row_id=row,action=actions[row],request_id=request_id,pagination_item_count=1,
                requested_utc='2026-09-06T18:00:%02dZ'%(2*index+1),completed_utc='2026-09-06T18:00:%02dZ'%(2*index+2))
            response={'Quota':{'ServiceCode':'ec2','QuotaCode':'L-1216C47A','Value':5 if row=='R34' else 4},
                'ResponseMetadata':{'RequestId':request_id,'HTTPStatusCode':200,'RetryAttempts':0,
                    'HTTPHeaders':{'x-amzn-requestid':request_id}}}
            R51ProspectiveAmendmentTests.encode(receipt,'request',request)
            R51ProspectiveAmendmentTests.encode(receipt,'response',response);receipts[row]=receipt
        context={k:base[k] for k in ('caller_identity','authentication_source_identity','freshness_max_seconds')}
        context.update(phase_not_before_utc='2026-09-06T18:00:00Z',validation_utc='2026-09-06T18:00:50Z')
        return receipts,context

    def test_actual_sources_reconstruct_applied_quota_without_substituting_default(self):
        receipts,context=self.material();output=G.build_service_quota_output(ROOT,receipts,**context)
        self.assertEqual(output['decoded_json']['quota_value'],5)
        self.assertEqual(output['decoded_json']['source_row_ids'],['R34','R35'])
        self.assertEqual(output['freshness_seconds'],49)
        self.assertEqual(F.validate_service_quota_reconstruction_output(output,receipts,**context),output)
        G.validate_record(ROOT,'service_quota_output',output)

    def test_target_value_order_and_source_context_drift_refuse(self):
        mutations=[('R34','request',lambda v:v.update(QuotaCode='other')),
            ('R34','response',lambda v:v['Quota'].update(ServiceCode='s3')),
            ('R34','response',lambda v:v['Quota'].update(Value=0)),
            ('R35','response',lambda v:v['Quota'].update(QuotaCode='other'))]
        for row,stem,mutate in mutations:
            receipts,context=self.material();R64IngressAmendmentTests().mutate_api(receipts[row],stem,mutate)
            with self.subTest(row=row,stem=stem),self.assertRaises(F.Refusal):
                F.build_service_quota_reconstruction_output(receipts,**context)
        for mutate in (lambda r:r.pop('R35'),lambda r:r['R35'].update(pagination_item_count=True),
            lambda r:r['R35'].update(requested_utc='2026-09-06T18:00:01Z'),
            lambda r:r['R34'].update(authentication_source_identity=F.identity('aws_authenticated_api_source/v1','d'*64))):
            receipts,context=self.material();mutate(receipts)
            with self.assertRaises(F.Refusal):F.build_service_quota_reconstruction_output(receipts,**context)

    def test_rehashed_output_or_stale_phase_cannot_replace_sources(self):
        receipts,context=self.material();output=G.build_service_quota_output(ROOT,receipts,**context)
        output['decoded_json']['quota_value']=4;raw=G.canonical(output['decoded_json'])
        output.update(identity=G.identity(output['schema'],output['decoded_json']),canonical_json_base64=base64.b64encode(raw).decode(),
            canonical_byte_sha256=G.sha(raw),canonical_byte_count=len(raw))
        with self.assertRaises(F.Refusal):F.validate_service_quota_reconstruction_output(output,receipts,**context)
        receipts,context=self.material();context['validation_utc']='2026-09-06T18:05:02Z'
        with self.assertRaises(F.Refusal):F.build_service_quota_reconstruction_output(receipts,**context)
...
class ReconstructionProgressV2Tests(unittest.TestCase):
    ...
    def material(self):
        ...
        receipts,quota_context=ServiceQuotaReconstructionTests().material()
        quota=G.build_service_quota_output(ROOT,receipts,**quota_context)
        return {'ACCOUNT_REGION':account,'INSTANCE_PROFILE_SOLE_ROLE':profile,'VPC_NETWORK_PATH':vpc,'SERVICE_QUOTA':quota}

    def test_four_canonical_candidates_bind_without_claiming_complete_set(self):
        ...
        self.assertEqual(record['candidate_control_ids_in_order'],
            ['ACCOUNT_REGION','INSTANCE_PROFILE_SOLE_ROLE','VPC_NETWORK_PATH','SERVICE_QUOTA'])
        self.assertEqual(len(record['unresolved_control_ids_in_order']),7)
    ...
    def test_unknown_output_and_rehashed_slot_mapping_refuse(self):
        with self.assertRaises(ValueError):G.build_runtime_control_reconstruction_progress_v2(ROOT,
            {'IAM_POLICY_SET':{}},phase='PREDEPLOYMENT',validation_utc='2026-09-06T18:00:50Z')
        record=G.build_runtime_control_reconstruction_progress_v2(ROOT,self.material(),
            phase='PREDEPLOYMENT',validation_utc='2026-09-06T18:00:50Z')
        plan=G.build_runtime_control_read_plan_fields_v2(ROOT)
...
class ReconstructionSourceAttachmentTests(unittest.TestCase):
    ...
    def material(self):
        ...
        quota_receipts,quota_context=ServiceQuotaReconstructionTests().material()
        quota=G.build_service_quota_output(ROOT,quota_receipts,**quota_context)
        ...
        return validation,sealed,sealed_id,{'candidate':account,'receipt':receipt,'context':account_context}, \
            ...
            {'candidate':quota,'receipts':quota_receipts,'context':quota_context}

    def test_source_attachment_reruns_all_four_reconstructors(self):
        validation,sealed,sealed_id,account,profile,vpc,quota=self.material()
        record=G.build_runtime_control_reconstruction_source_attachment_v1(ROOT,
            ...
            service_quota_bundle=quota,sealed_context_identity=sealed_id,validation_utc=validation)
    ...
    def test_rehashed_candidate_or_source_context_refuses(self):
        ...
        with self.assertRaises(F.Refusal):F.build_runtime_control_reconstruction_source_attachment_v1(
            ...
            instance_profile_bundle=profile,vpc_bundle=vpc,service_quota_bundle=quota,sealed_context=sealed,sealed_context_identity=sealed_id)
    ...
    def test_public_validator_rejects_rehashed_replacement_context(self):
        ...
        with self.assertRaises(F.Refusal):F.build_runtime_control_reconstruction_source_attachment_v1(
            ...
            phase='PREDEPLOYMENT',validation_utc=validation,account_bundle=account,
            instance_profile_bundle=profile,vpc_bundle=vpc,service_quota_bundle=quota,sealed_context=sealed,sealed_context_identity=sealed_id)
        validation,sealed,sealed_id,account,profile,vpc,quota=self.material();vpc['context']=copy.deepcopy(vpc['context'])
        ...
        with self.assertRaises(F.Refusal):F.build_runtime_control_reconstruction_source_attachment_v1(
            ...
            instance_profile_bundle=profile,vpc_bundle=vpc,service_quota_bundle=quota,sealed_context=sealed,sealed_context_identity=sealed_id)
        validation,sealed,sealed_id,account,profile,vpc,quota=self.material()
        quota['context']=copy.deepcopy(quota['context']);quota['context']['phase_not_before_utc']='2026-09-06T18:00:02Z'
        with self.assertRaises(F.Refusal):F.build_runtime_control_reconstruction_source_attachment_v1(
            read_plan=plan['runtime_control_read_plan'],read_plan_identity=plan['runtime_control_read_plan_identity'],
            phase='PREDEPLOYMENT',validation_utc=validation,account_bundle=account,
            instance_profile_bundle=profile,vpc_bundle=vpc,service_quota_bundle=quota,sealed_context=sealed,sealed_context_identity=sealed_id)
...
class LocalHelperSourceTests(unittest.TestCase):
    def material(self):
        ...
        packet={'schema':'aws_c0_live_packet/v9','launch_request_identity':C.identity('aws_c0_launch_request/v9',C.root_digest(launch)),
        ...
        observed={'Name':'EBU-C0-Start-v1','DocumentVersion':'1','Status':'Active','DocumentType':'Command'}
        auth={'schema':'aws_c0_live_authorization/v9','live_packet_identity':C.identity('aws_c0_live_packet/v9',C.digest(raws['live_packet'])),
...
def current_launch(record):
    ...
    result["schema"] = "aws_c0_launch_request/v9"
    result["preparation_packet_identity"]["kind"] = "aws_c0_preparation_packet/v9"
    result["preparation_authorization_identity"]["kind"] = "aws_c0_preparation_authorization/v8"
...
class DeploymentSequenceTests(unittest.TestCase):
    ...
    def rebind(packet,auth,seed,launch):
        ...
        packet['launch_request_identity']=F.identity('aws_c0_launch_request/v9',launch['record_sha256'])
        packet['launch_request_object']=DeploymentSequenceTests.receipt(launch,'launch')
        auth['live_packet_identity']=F.identity('aws_c0_live_packet/v9',F.digest(F.canonical_bytes(packet)))
    ...
    def material(self):
        ...
        packet={'schema':'aws_c0_live_packet/v9','observed_utc':'2026-09-06T18:00:00Z',
        ...
        auth={'schema':'aws_c0_live_authorization/v9','observed_utc':'2026-09-06T18:05:00Z',
    ...
    def complete_material(self):
        ...
        packet.update(packet_disposition='AWS_C0_LIVE_PACKET_COMPLETE_UNAUTHORIZED',
            preparation_closure_identity=F.identity('aws_c0_preparation_closure/v8','4'*64),
    ...
    def test_v9_carriers_bind_source_attachment_and_preserve_nine_then_eleven_controls(self):
        packet,auth,_,launch=self.complete_material()
        self.assertEqual(G.validate_record(ROOT,'launch_v9',launch),launch)
        self.assertEqual(F._validate_launch_v9(launch),launch)
        self.assertEqual(G.validate_record(ROOT,'live_packet_v9',packet),packet)
        self.assertEqual(F._validate_live_packet_v9(packet),None)
        self.assertEqual(G.validate_record(ROOT,'live_authorization_v9',auth),auth)
        self.assertEqual(F._validate_live_authorization_v9(auth),None)
    ...
    def test_v9_carrier_refuses_attachment_or_change_set_projection_substitution(self):
        for mutate in (
            ...
            with self.subTest(mutate=mutate),self.assertRaises(F.Refusal):F._validate_live_packet_v9(packet)
    ...
    def test_new_schema_derivation_is_offline_deterministic_and_keeps_history(self):
        ...
        prospective=G.record_schema(ROOT,'preparation_packet_v9')['allOf'][1]
        self.assertEqual(prospective['properties']['planned_pre_live_record_kinds']['const'],
                         list(V.SEQUENCE_PRELIVE_RECORD_KINDS))
        self.assertEqual(G.record_schema(ROOT,'preparation_closure_v8')['allOf'][1]
                         ['properties']['launch_request_identity']['allOf'][1]
                         ['properties']['kind']['const'],'aws_c0_launch_request/v9')
...
class AuthorityTests(unittest.TestCase):
    ...
    def test_overnight_scope_addition_remains_exact_and_refuses_unrelated_paths(self):
        ...
        self.assertIn('AWS_C0_RUNTIME_ARTIFACT_COMPATIBILITY_SUCCESSOR_AMENDMENT.md',
                      V.LOCAL_DEPLOYMENT_READINESS_PATHS)
        self.assertIn('aws_c0_runtime_artifact_compatibility_successor_contract.json',
                      V.LOCAL_DEPLOYMENT_READINESS_PATHS)
...
class Gate1LineageCorrectionTests(unittest.TestCase):
    ...
    def test_c0_g1_lineage_n06_same_kind_wrong_hash_refused(self):
        ...
        packet = self.packet()
        packet['schema'] = 'aws_c0_live_packet/v9'
        packet["pre_live_predecessor_object_receipts"] = artifacts + records
        auth = {"live_packet_identity": C.identity("aws_c0_live_packet/v9", "c" * 64)}
        ...
        with (mock.patch.object(F, "_fetch_receipt", side_effect=[
              ...
              mock.patch.object(F, "_validate_live_packet_v9"),
              mock.patch.object(F, "_validate_live_authorization_v9"),
