...
**Definition 6.4 (unavoidable action burden).** `C_a : [0, q_acc] → ℝ≥0` is a
declared, non-negative, committed-before-execution function representing the
unavoidable local burden of physically performing the action (activation,
transport dissipation priced into burden units, wear). Requirements:
`C_a(q) ≥ 0`; `C_a(0) = 0` for the no-action point (a fixed activation
component `c₀·1[q>0]` is permitted for `q > 0`); `C_a` is declared and audited,
never negative (a negative `C_a` is a subsidy that breaks §8). A candidate
consistent with the V2.5 debits is `C_a(q) = c₀·1[q>0] + λ_L·Δt(1−η_e)q`, with
`λ_L` a declared burden-per-stock conversion weight.
...
Moreover `q ↦ V_loc(z) − ψ(q)` is concave, so the exact quote schedule is
concave (net of a convex `C_a`).
...
sides. Concavity of `−ψ` is immediate. ∎
...
a point on the committed schedule — never a new valuation rule. Concavity
(Thm 6.9) guarantees partial execution is never over-credited.
...
The statement is exact, not asymptotic. What it does *not* cover are the edge
cases below, each of which perturbs an assumption.
...
**Corollary 8.3 (bounded issuance).** Under (T1)–(T6), cumulative net EBU
issued over any sequence is at most `V(x⁰) − inf V ≤ V(x⁰)`: a region's
initial burden is a hard ceiling on extractable EBU. There is no perpetual
EBU source without physical burden reduction.
...
Only after Gate 1 conformance should any Gate 2 (multi-actor allocation, O3)
or accounting-layer work (debt vector, wallets) be considered, in that order.
...
*End of V3.0 Gate 0 design note. No code, test, experiment, result, manifest,
README, CI, CITATION, PDF, or released artifact was created or modified by
this gate other than this single new file.*
