...
FROZEN_PRELIVE_RECORD_KINDS = (
    ...
    "aws_c0_preparation_packet/v9",
    "aws_c0_preparation_authorization/v8",
    ...
    "aws_c0_launch_request/v9",
    "aws_c0_preparation_closure/v8",
)
...
ROOT_KINDS = (
    ...
    "aws_c0_material_runtime_static_validation/v4",
    "aws_c0_private_infrastructure_snapshot/v1",
    "aws_c0_launch_request/v9", "aws_c0_start_receipt/v7", "aws_c0_heartbeat/v1",
...
CURRENT_PHASE_KINDS = {'aws_c0_closure_seed/v2','aws_c0_preparation_packet/v9','aws_c0_preparation_authorization/v8',
                       'aws_c0_launch_request/v9','aws_c0_preparation_closure/v8','aws_c0_live_packet/v9','aws_c0_live_authorization/v9'}
...
LIVE_PACKET_V9_FIELDS = LIVE_PACKET_V6_FIELDS | {
...
LIVE_AUTH_V9_FIELDS = LIVE_AUTH_V6_FIELDS | {"runtime_control_reconstruction_source_attachment_identity"}
...
LAUNCH_V9_FIELDS = LAUNCH_V6_FIELDS
CATEGORY = {
...
def _validate_launch(record: Any, schema: str, fields: set[str], packet_kind: str,
                     authorization_kind: str) -> dict[str, Any]:
    if not isinstance(record, dict) or set(record) != fields:
        raise Refusal("launch-v5 field closure failed")
    _common(record,schema,root=True)
    ...
    _identity(record, "preparation_packet_identity", packet_kind)
    _identity(record, "preparation_authorization_identity", authorization_kind)
...
def _validate_launch_v6(record: Any) -> dict[str, Any]:
    return _validate_launch(record,'aws_c0_launch_request/v6',LAUNCH_V6_FIELDS,
                            'aws_c0_preparation_packet/v5','aws_c0_preparation_authorization/v4')


def _validate_launch_v9(record: Any) -> dict[str, Any]:
    return _validate_launch(record,'aws_c0_launch_request/v9',LAUNCH_V9_FIELDS,
                            'aws_c0_preparation_packet/v9','aws_c0_preparation_authorization/v8')
...
PREDEPLOYMENT_CONTROLS = tuple(k for k in POST_DEPLOYMENT_CONTROLS
                              if k not in {"STANDARD_WORKFLOW", "SSM_DOCUMENT"})
PREDEPLOYMENT_SOURCE_CONTROLS = tuple(k for k in PREDEPLOYMENT_CONTROLS
                                     if k != "SOFTWARE_AND_IMAGE_SET")
...
def validate_deployment_sequence(packet: dict[str, Any], auth: dict[str, Any], seed: dict[str, Any],
    ...
    for parent, child, id_field, receipt_field, kind, root in (
        (auth, packet, 'live_packet_identity', 'live_packet_object', 'aws_c0_live_packet/v9', False),
        (packet, launch, 'launch_request_identity', 'launch_request_object', 'aws_c0_launch_request/v9', True),
    ...
    if auth.get('runtime_control_reconstruction_source_attachment_identity') != packet.get(
            'runtime_control_reconstruction_source_attachment_identity'):
        raise Refusal('deployment phase source attachment mismatch')
...
RECONSTRUCTION_PROGRESS_OUTPUT_KINDS_V5={**RECONSTRUCTION_PROGRESS_OUTPUT_KINDS_V4,
    'ARTIFACT_VERSION_SET':'aws_c0_artifact_version_set_observation/v1'}
RECONSTRUCTION_PROGRESS_OUTPUT_KINDS_V6={**RECONSTRUCTION_PROGRESS_OUTPUT_KINDS_V5,
    'CHANGE_SET_AND_EFFECTS':'aws_c0_change_set_effects_observation/v1'}
...
def validate_runtime_control_reconstruction_progress_v3(record: Any, *, read_plan: Any,
        read_plan_identity: dict[str,str], phase: str, validation_utc: str) -> dict[str,Any]:
    """Validate the five-candidate envelope bound to read-plan/v3."""
    fields={'schema','read_plan_identity','phase','observed_utc','controls_in_order',
        'candidate_control_ids_in_order','unresolved_control_ids_in_order',
        'source_revalidation_performed','complete_reconstruction_claimed','disposition'}
    if (not isinstance(record,dict) or set(record)!=fields
            or record.get('schema')!='aws_c0_runtime_control_reconstruction_progress/v3'
            or phase not in ('PREDEPLOYMENT','POSTDEPLOYMENT','EXECUTION_PREFLIGHT','COMPLETION')
        ...
        raise Refusal('closed v3 partial reconstruction progress envelope required')
    validate_runtime_control_read_plan_v3(read_plan,read_plan_identity)
    rows=read_plan['required_control_mapping']
    if [item.get('control') if isinstance(item,dict) else None for item in rows]!=list(RECONSTRUCTION_PROGRESS_CONTROLS):
        raise Refusal('exact eleven-control v3 plan ordering required')
    entries=record['controls_in_order']
    if not isinstance(entries,list) or len(entries)!=len(rows):
        raise Refusal('exact eleven v3 partial reconstruction slots required')
    completed=[];unresolved=[]
    for plan_entry,entry in zip(rows,entries):
        if not isinstance(entry,dict) or set(entry)!={'control','mapped_row_ids','state','output','output_identity'}:
            raise Refusal('v3 partial reconstruction slot field closure failed')
        control=plan_entry['control'];kind=RECONSTRUCTION_PROGRESS_OUTPUT_KINDS_V3.get(control)
        if entry['control']!=control or entry['mapped_row_ids']!=plan_entry['row_ids']:
            raise Refusal('v3 partial reconstruction control mapping drift')
        if entry['state']=='CANONICAL_OUTPUT_CANDIDATE':
            output=entry['output']
            if not kind or not isinstance(output,dict) or output.get('schema')!=kind or output.get('kind')!=kind:
                raise Refusal('unsupported or malformed v3 partial output')
            decoded=output.get('decoded_json');raw=canonical_bytes(decoded)
            if (output.get('identity')!=identity(kind,digest(raw))
                    or output.get('canonical_json_base64')!=base64.b64encode(raw).decode()
                    or output.get('canonical_byte_sha256')!=digest(raw)
                    ...
                    or decoded.get('source_row_ids')!=plan_entry['row_ids']):
                raise Refusal('v3 partial output canonical/source binding differs')
            completed.append(control)
        elif entry['state']=='UNRESOLVED':
            if entry.get('output') is not None or entry.get('output_identity') is not None:
                raise Refusal('v3 unresolved partial slot cannot claim output')
            unresolved.append(control)
        else:raise Refusal('v3 partial reconstruction slot state refused')
    if (record['candidate_control_ids_in_order']!=completed
...
def validate_runtime_control_reconstruction_progress_v5(record: Any, *, read_plan: Any,
        read_plan_identity: dict[str,str], phase: str, validation_utc: str) -> dict[str,Any]:
    """Validate the seven-candidate envelope bound to read-plan/v3."""
    fields={'schema','read_plan_identity','phase','observed_utc','controls_in_order',
        'candidate_control_ids_in_order','unresolved_control_ids_in_order',
        'source_revalidation_performed','complete_reconstruction_claimed','disposition'}
    if (not isinstance(record,dict) or set(record)!=fields
            or record.get('schema')!='aws_c0_runtime_control_reconstruction_progress/v5'
            or phase not in ('PREDEPLOYMENT','POSTDEPLOYMENT','EXECUTION_PREFLIGHT','COMPLETION')
            or record.get('phase')!=phase or record.get('observed_utc')!=validation_utc
            or record.get('read_plan_identity')!=read_plan_identity
            or record.get('source_revalidation_performed') is not False
            or record.get('complete_reconstruction_claimed') is not False
            or record.get('disposition')!='PARTIAL_NOT_READY'):
        raise Refusal('closed v5 partial reconstruction progress envelope required')
    validate_runtime_control_read_plan_v3(read_plan,read_plan_identity)
    rows=read_plan['required_control_mapping']
    if [item.get('control') if isinstance(item,dict) else None for item in rows]!=list(RECONSTRUCTION_PROGRESS_CONTROLS):
        raise Refusal('exact eleven-control v5 plan ordering required')
    entries=record['controls_in_order']
    if not isinstance(entries,list) or len(entries)!=len(rows):
        raise Refusal('exact eleven v5 partial reconstruction slots required')
    completed=[];unresolved=[]
    for plan_entry,entry in zip(rows,entries):
        if not isinstance(entry,dict) or set(entry)!={'control','mapped_row_ids','state','output','output_identity'}:
            raise Refusal('v5 partial reconstruction slot field closure failed')
        control=plan_entry['control'];kind=RECONSTRUCTION_PROGRESS_OUTPUT_KINDS_V5.get(control)
        if entry['control']!=control or entry['mapped_row_ids']!=plan_entry['row_ids']:
            raise Refusal('v5 partial reconstruction control mapping drift')
        if entry['state']=='CANONICAL_OUTPUT_CANDIDATE':
            output=entry['output']
            if not kind or not isinstance(output,dict) or output.get('schema')!=kind or output.get('kind')!=kind:
                raise Refusal('unsupported or malformed v5 partial output')
            decoded=output.get('decoded_json');raw=canonical_bytes(decoded)
            if (output.get('identity')!=identity(kind,digest(raw))
                    or output.get('canonical_json_base64')!=base64.b64encode(raw).decode()
                    or output.get('canonical_byte_sha256')!=digest(raw)
                    ...
                    or entry.get('output_identity')!=output['identity']
                    or decoded.get('source_row_ids')!=plan_entry['row_ids']):
                raise Refusal('v5 partial output canonical/source binding differs')
            completed.append(control)
        elif entry['state']=='UNRESOLVED':
            if entry.get('output') is not None or entry.get('output_identity') is not None:
                raise Refusal('v5 unresolved partial slot cannot claim output')
            unresolved.append(control)
        else:raise Refusal('v5 partial reconstruction slot state refused')
    if (record['candidate_control_ids_in_order']!=completed
            or record['unresolved_control_ids_in_order']!=unresolved or not unresolved):
        raise Refusal('v5 partial reconstruction completion inventory differs')
    return record


def validate_runtime_control_reconstruction_progress_v6(record: Any, *, read_plan: Any,
        read_plan_identity: dict[str,str], phase: str, validation_utc: str) -> dict[str,Any]:
    """Validate the eight-candidate envelope bound to read-plan/v3."""
    fields={'schema','read_plan_identity','phase','observed_utc','controls_in_order',
        'candidate_control_ids_in_order','unresolved_control_ids_in_order',
        'source_revalidation_performed','complete_reconstruction_claimed','disposition'}
    if (not isinstance(record,dict) or set(record)!=fields
            or record.get('schema')!='aws_c0_runtime_control_reconstruction_progress/v6'
            or phase not in ('PREDEPLOYMENT','POSTDEPLOYMENT','EXECUTION_PREFLIGHT','COMPLETION')
            or record.get('phase')!=phase or record.get('observed_utc')!=validation_utc
            or record.get('read_plan_identity')!=read_plan_identity
            or record.get('source_revalidation_performed') is not False
            or record.get('complete_reconstruction_claimed') is not False
            or record.get('disposition')!='PARTIAL_NOT_READY'):
        raise Refusal('closed v6 partial reconstruction progress envelope required')
    validate_runtime_control_read_plan_v3(read_plan,read_plan_identity)
    rows=read_plan['required_control_mapping']
    if [item.get('control') if isinstance(item,dict) else None for item in rows]!=list(RECONSTRUCTION_PROGRESS_CONTROLS):
        raise Refusal('exact eleven-control v6 plan ordering required')
    entries=record['controls_in_order']
    if not isinstance(entries,list) or len(entries)!=len(rows):
        raise Refusal('exact eleven v6 partial reconstruction slots required')
    completed=[];unresolved=[]
    for plan_entry,entry in zip(rows,entries):
        if not isinstance(entry,dict) or set(entry)!={'control','mapped_row_ids','state','output','output_identity'}:
            raise Refusal('v6 partial reconstruction slot field closure failed')
        control=plan_entry['control'];kind=RECONSTRUCTION_PROGRESS_OUTPUT_KINDS_V6.get(control)
        if entry['control']!=control or entry['mapped_row_ids']!=plan_entry['row_ids']:
            raise Refusal('v6 partial reconstruction control mapping drift')
        if entry['state']=='CANONICAL_OUTPUT_CANDIDATE':
            output=entry['output']
            if not kind or not isinstance(output,dict) or output.get('schema')!=kind or output.get('kind')!=kind:
                raise Refusal('unsupported or malformed v6 partial output')
            decoded=output.get('decoded_json');raw=canonical_bytes(decoded)
            if (output.get('identity')!=identity(kind,digest(raw))
                    or output.get('canonical_json_base64')!=base64.b64encode(raw).decode()
                    or output.get('canonical_byte_sha256')!=digest(raw)
                    ...
                    or entry.get('output_identity')!=output['identity']
                    or decoded.get('source_row_ids')!=plan_entry['row_ids']):
                raise Refusal('v6 partial output canonical/source binding differs')
            completed.append(control)
        elif entry['state']=='UNRESOLVED':
            if entry.get('output') is not None or entry.get('output_identity') is not None:
                raise Refusal('v6 unresolved partial slot cannot claim output')
            unresolved.append(control)
        else:raise Refusal('v6 partial reconstruction slot state refused')
    if (record['candidate_control_ids_in_order']!=completed
            or record['unresolved_control_ids_in_order']!=unresolved or not unresolved):
        raise Refusal('v6 partial reconstruction completion inventory differs')
    return record
...
def _read_plan_v3_predecessor(read_plan: Any, read_plan_identity: dict[str,str]) -> tuple[dict[str,Any],dict[str,str]]:
    validate_runtime_control_read_plan_v3(read_plan,read_plan_identity)
    fields={'schema','rows','row_ids','required_control_mapping','shared_row_ids','map_sha256',
        'plan_contract_identity','freshness_max_seconds','original_read_plan_identity','amendment_packet_identity'}
    mapping=strict_json(canonical_bytes(read_plan['required_control_mapping']))
    target=next(item for item in mapping if item.get('control')=='IAM_POLICY_SET')
    target['output_schema']=target['output_kind']='aws_c0_iam_policy_set_observation/v1'
    prior={key:read_plan[key] for key in fields}
    prior.update(schema='aws_c0_runtime_control_read_plan/v2',required_control_mapping=mapping,
        map_sha256=digest(canonical_bytes(mapping)))
    validate_runtime_control_read_plan_v2(prior,read_plan['previous_read_plan_identity'])
    return prior,read_plan['previous_read_plan_identity']


def build_runtime_control_reconstruction_source_attachment_v2(*, read_plan: Any,
        read_plan_identity: dict[str,str], phase: str, validation_utc: str,
        sealed_context: Any, sealed_context_identity: dict[str,str], account_bundle: Any,
        instance_profile_bundle: Any, iam_bundle: Any, vpc_bundle: Any,
        service_quota_bundle: Any) -> dict[str,Any]:
    """Extend the exact v1 attachment with authenticated IAM reconstruction."""
    if phase!='PREDEPLOYMENT':raise Refusal('only five implemented predeployment controls may attach')
    prior_plan,prior_plan_identity=_read_plan_v3_predecessor(read_plan,read_plan_identity)
    context_fields={'schema','phase_not_before_utc','validation_utc','freshness_max_seconds',
        'caller_identity','authentication_source_identity','expected_caller_arn','expected_caller_user_id',
        'expected_role_id','attempt_identity','previous_budget_identity','ec2_pagination_bounds','iam_pagination_bounds'}
    if (not isinstance(sealed_context,dict) or set(sealed_context)!=context_fields
            or sealed_context.get('schema')!='aws_c0_predeployment_reconstruction_context/v2'
            or sealed_context.get('validation_utc')!=validation_utc
            or sealed_context.get('freshness_max_seconds')!=read_plan['freshness_max_seconds']
            or sealed_context_identity!=identity(sealed_context['schema'],digest(canonical_bytes(sealed_context)))):
        raise Refusal('v2 independently supplied sealed reconstruction context required')
    prior_context=strict_json(canonical_bytes(sealed_context));prior_context['schema']='aws_c0_predeployment_reconstruction_context/v1'
    del prior_context['iam_pagination_bounds']
    prior_context_identity=identity(prior_context['schema'],digest(canonical_bytes(prior_context)))
    prior=build_runtime_control_reconstruction_source_attachment_v1(read_plan=prior_plan,
        read_plan_identity=prior_plan_identity,phase=phase,validation_utc=validation_utc,
    ...
    if not isinstance(iam_bundle,dict) or set(iam_bundle)!={'candidate','receipts','role_context_binding','context'}:
        raise Refusal('IAM source bundle field closure failed')
    context=iam_bundle['context']
    if (not isinstance(context,dict)
            ...
            or context.get('iam_pagination_bounds')!=sealed_context['iam_pagination_bounds']
            or context.get('read_plan')!=read_plan or context.get('read_plan_identity')!=read_plan_identity):
        raise Refusal('IAM source bundle context differs from sealed v2 context')
    expected_iam=build_iam_policy_set_reconstruction_output_v2(iam_bundle['receipts'],
        iam_bundle['role_context_binding'],instance_profile_bundle,**context)
    if canonical_bytes(iam_bundle['candidate'])!=canonical_bytes(expected_iam):
        raise Refusal('IAM source attachment candidate differs from rerun reconstruction')
    outputs=list(prior['outputs_in_order']);outputs.insert(2,expected_iam)
    bundles=list(prior['source_bundles_in_order']);bundles.insert(2,iam_bundle)
    record={'schema':'aws_c0_runtime_control_reconstruction_source_attachment/v2',
    ...
    controls=('ACCOUNT_REGION','INSTANCE_PROFILE_SOLE_ROLE','IAM_POLICY_SET','VPC_NETWORK_PATH','SERVICE_QUOTA')
    if [item.get('control') for item in outputs]!=list(controls):
        raise Refusal('v2 source attachment output control order differs')
    return record


def validate_runtime_control_reconstruction_source_attachment_v2(record: Any, *, read_plan: Any,
        read_plan_identity: dict[str,str], phase: str, validation_utc: str,
    ...
    fields={'schema','read_plan_identity','previous_source_attachment_identity','phase','validation_utc',
        ...
        'source_revalidation_performed','complete_reconstruction_claimed','disposition'}
    if (not isinstance(record,dict) or set(record)!=fields
            or record.get('schema')!='aws_c0_runtime_control_reconstruction_source_attachment/v2'
            ...
            or len(record['source_bundles_in_order'])!=5):
        raise Refusal('closed v2 source-bound partial attachment required')
    bundles=record['source_bundles_in_order']
    expected=build_runtime_control_reconstruction_source_attachment_v2(read_plan=read_plan,
        read_plan_identity=read_plan_identity,phase=phase,validation_utc=validation_utc,
...
def build_runtime_control_reconstruction_source_attachment_v4(*, read_plan: Any,
        read_plan_identity: dict[str,str], phase: str, validation_utc: str,
        sealed_context: Any, sealed_context_identity: dict[str,str], account_bundle: Any,
        instance_profile_bundle: Any, iam_bundle: Any, bucket_controls_kms_bundle: Any,
        vpc_bundle: Any, service_quota_bundle: Any, artifact_version_set_bundle: Any) -> dict[str,Any]:
    """Extend the exact v3 attachment with authenticated exact artifact versions."""
    if phase!='PREDEPLOYMENT':raise Refusal('only seven implemented predeployment controls may attach')
    context_fields={'schema','phase_not_before_utc','validation_utc','freshness_max_seconds',
        'caller_identity','authentication_source_identity','expected_caller_arn','expected_caller_user_id',
        'expected_role_id','attempt_identity','previous_budget_identity','ec2_pagination_bounds',
        'iam_pagination_bounds','expected_bucket_name','expected_bucket_identity','expected_prefix',
        's3_pagination_bounds'}
    if (not isinstance(sealed_context,dict) or set(sealed_context)!=context_fields
            or sealed_context.get('schema')!='aws_c0_predeployment_reconstruction_context/v4'
            or sealed_context.get('validation_utc')!=validation_utc
            or sealed_context.get('freshness_max_seconds')!=read_plan['freshness_max_seconds']
            or sealed_context_identity!=identity(sealed_context['schema'],digest(canonical_bytes(sealed_context)))):
        raise Refusal('v4 independently supplied sealed reconstruction context required')
    prior_context=strict_json(canonical_bytes(sealed_context));prior_context['schema']='aws_c0_predeployment_reconstruction_context/v3'
    del prior_context['expected_prefix'];del prior_context['s3_pagination_bounds']
    prior_context_identity=identity(prior_context['schema'],digest(canonical_bytes(prior_context)))
    prior=build_runtime_control_reconstruction_source_attachment_v3(read_plan=read_plan,
        read_plan_identity=read_plan_identity,phase=phase,validation_utc=validation_utc,
        sealed_context=prior_context,sealed_context_identity=prior_context_identity,
        account_bundle=account_bundle,instance_profile_bundle=instance_profile_bundle,
        iam_bundle=iam_bundle,bucket_controls_kms_bundle=bucket_controls_kms_bundle,
        vpc_bundle=vpc_bundle,service_quota_bundle=service_quota_bundle)
    if (not isinstance(artifact_version_set_bundle,dict)
            or set(artifact_version_set_bundle)!={'candidate','receipts','context'}):
        raise Refusal('artifact source bundle field closure failed')
    context=artifact_version_set_bundle['context']
    if (not isinstance(context,dict)
            or context.get('caller_identity')!=sealed_context['caller_identity']
            or context.get('authentication_source_identity')!=sealed_context['authentication_source_identity']
            or context.get('phase_not_before_utc')!=sealed_context['phase_not_before_utc']
            ...
            or context.get('expected_bucket_name')!=sealed_context['expected_bucket_name']
            or context.get('expected_bucket_identity')!=sealed_context['expected_bucket_identity']
            or context.get('expected_prefix')!=sealed_context['expected_prefix']
            or context.get('s3_pagination_bounds')!=sealed_context['s3_pagination_bounds']):
        raise Refusal('artifact source bundle context differs from sealed v4 context')
    expected_artifact=build_artifact_version_set_reconstruction_output(
        artifact_version_set_bundle['receipts'],**context)
    if canonical_bytes(artifact_version_set_bundle['candidate'])!=canonical_bytes(expected_artifact):
        raise Refusal('artifact source attachment candidate differs from rerun reconstruction')
    outputs=list(prior['outputs_in_order'])+[expected_artifact]
    bundles=list(prior['source_bundles_in_order'])+[artifact_version_set_bundle]
    record={'schema':'aws_c0_runtime_control_reconstruction_source_attachment/v4',
        'read_plan_identity':read_plan_identity,'previous_source_attachment_identity':identity(
            prior['schema'],digest(canonical_bytes(prior))),
        'phase':phase,'validation_utc':validation_utc,'sealed_context':sealed_context,
        'sealed_context_identity':sealed_context_identity,'source_bundles_in_order':bundles,
    ...
    if [item.get('control') for item in outputs]!=list(controls):
        raise Refusal('v4 source attachment output control order differs')
    return record


def validate_runtime_control_reconstruction_source_attachment_v4(record: Any, *, read_plan: Any,
        read_plan_identity: dict[str,str], phase: str, validation_utc: str,
        expected_sealed_context_identity: dict[str,str]) -> dict[str,Any]:
    fields={'schema','read_plan_identity','previous_source_attachment_identity','phase','validation_utc',
        'sealed_context','sealed_context_identity','source_bundles_in_order','outputs_in_order',
        'source_revalidation_performed','complete_reconstruction_claimed','disposition'}
    if (not isinstance(record,dict) or set(record)!=fields
            or record.get('schema')!='aws_c0_runtime_control_reconstruction_source_attachment/v4'
            or record.get('read_plan_identity')!=read_plan_identity or record.get('phase')!=phase
            or record.get('validation_utc')!=validation_utc
            or record.get('sealed_context_identity')!=expected_sealed_context_identity
            or record.get('source_revalidation_performed') is not True
            ...
            or record.get('disposition')!='PARTIAL_SOURCE_BOUND_NOT_READY'
            or not isinstance(record.get('source_bundles_in_order'),list)
            or len(record['source_bundles_in_order'])!=7):
        raise Refusal('closed v4 source-bound partial attachment required')
    bundles=record['source_bundles_in_order']
    expected=build_runtime_control_reconstruction_source_attachment_v4(read_plan=read_plan,
        read_plan_identity=read_plan_identity,phase=phase,validation_utc=validation_utc,
        sealed_context=record['sealed_context'],sealed_context_identity=record['sealed_context_identity'],
        account_bundle=bundles[0],instance_profile_bundle=bundles[1],iam_bundle=bundles[2],
        bucket_controls_kms_bundle=bundles[3],vpc_bundle=bundles[4],service_quota_bundle=bundles[5],
        artifact_version_set_bundle=bundles[6])
    if canonical_bytes(record)!=canonical_bytes(expected):
        raise Refusal('v4 source-bound attachment differs from rerun exact reconstruction')
    return record


def build_runtime_control_reconstruction_source_attachment_v5(*, read_plan: Any,
        read_plan_identity: dict[str,str], phase: str, validation_utc: str,
        sealed_context: Any, sealed_context_identity: dict[str,str], account_bundle: Any,
        instance_profile_bundle: Any, iam_bundle: Any, bucket_controls_kms_bundle: Any,
        vpc_bundle: Any, service_quota_bundle: Any, change_set_effects_bundle: Any,
        artifact_version_set_bundle: Any) -> dict[str,Any]:
    """Extend v4 with the authenticated predeployment CREATE change set."""
    if phase!='PREDEPLOYMENT':raise Refusal('only eight implemented predeployment controls may attach')
    added={'expected_change_set_arn','expected_stack_name','expected_template_sha256',
        'expected_effect_actions_in_order','cfn_pagination_bounds'}
    prior_fields={'schema','phase_not_before_utc','validation_utc','freshness_max_seconds',
        'caller_identity','authentication_source_identity','expected_caller_arn','expected_caller_user_id',
        'expected_role_id','attempt_identity','previous_budget_identity','ec2_pagination_bounds',
        'iam_pagination_bounds','expected_bucket_name','expected_bucket_identity','expected_prefix',
        's3_pagination_bounds'}
    if (not isinstance(sealed_context,dict) or set(sealed_context)!=prior_fields|added
            or sealed_context.get('schema')!='aws_c0_predeployment_reconstruction_context/v5'
            or sealed_context.get('validation_utc')!=validation_utc
            or sealed_context.get('freshness_max_seconds')!=read_plan['freshness_max_seconds']
            or sealed_context_identity!=identity(sealed_context['schema'],digest(canonical_bytes(sealed_context)))):
        raise Refusal('v5 independently supplied sealed reconstruction context required')
    prior_context=strict_json(canonical_bytes(sealed_context));prior_context['schema']='aws_c0_predeployment_reconstruction_context/v4'
    for field in added:del prior_context[field]
    prior_context_identity=identity(prior_context['schema'],digest(canonical_bytes(prior_context)))
    prior=build_runtime_control_reconstruction_source_attachment_v4(read_plan=read_plan,
        read_plan_identity=read_plan_identity,phase=phase,validation_utc=validation_utc,
        sealed_context=prior_context,sealed_context_identity=prior_context_identity,
        account_bundle=account_bundle,instance_profile_bundle=instance_profile_bundle,
        iam_bundle=iam_bundle,bucket_controls_kms_bundle=bucket_controls_kms_bundle,
        vpc_bundle=vpc_bundle,service_quota_bundle=service_quota_bundle,
        artifact_version_set_bundle=artifact_version_set_bundle)
    if (not isinstance(change_set_effects_bundle,dict)
            or set(change_set_effects_bundle)!={'candidate','receipts','context'}):
        raise Refusal('change-set source bundle field closure failed')
    context=change_set_effects_bundle['context']
    if (not isinstance(context,dict)
            or any(context.get(field)!=sealed_context[field] for field in
                ('caller_identity','authentication_source_identity','phase_not_before_utc','validation_utc',
                 'freshness_max_seconds','expected_change_set_arn','expected_stack_name','expected_template_sha256',
                 'expected_effect_actions_in_order','cfn_pagination_bounds'))):
        raise Refusal('change-set source bundle context differs from sealed v5 context')
    expected_change=build_change_set_effects_reconstruction_output(change_set_effects_bundle['receipts'],**context)
    if canonical_bytes(change_set_effects_bundle['candidate'])!=canonical_bytes(expected_change):
        raise Refusal('change-set source attachment candidate differs from rerun reconstruction')
    outputs=list(prior['outputs_in_order']);outputs.insert(-1,expected_change)
    bundles=list(prior['source_bundles_in_order']);bundles.insert(-1,change_set_effects_bundle)
    record={'schema':'aws_c0_runtime_control_reconstruction_source_attachment/v5',
        'read_plan_identity':read_plan_identity,'previous_source_attachment_identity':identity(
            prior['schema'],digest(canonical_bytes(prior))),
        'phase':phase,'validation_utc':validation_utc,'sealed_context':sealed_context,
        'sealed_context_identity':sealed_context_identity,'source_bundles_in_order':bundles,
        ...
        'complete_reconstruction_claimed':False,'disposition':'PARTIAL_SOURCE_BOUND_NOT_READY'}
    controls=('ACCOUNT_REGION','INSTANCE_PROFILE_SOLE_ROLE','IAM_POLICY_SET','BUCKET_CONTROLS_KMS',
        'VPC_NETWORK_PATH','SERVICE_QUOTA','CHANGE_SET_AND_EFFECTS','ARTIFACT_VERSION_SET')
    if [item.get('control') for item in outputs]!=list(controls):
        raise Refusal('v5 source attachment output control order differs')
    return record


def validate_runtime_control_reconstruction_source_attachment_v5(record: Any, *, read_plan: Any,
        read_plan_identity: dict[str,str], phase: str, validation_utc: str,
        expected_sealed_context_identity: dict[str,str]) -> dict[str,Any]:
    fields={'schema','read_plan_identity','previous_source_attachment_identity','phase','validation_utc',
        'sealed_context','sealed_context_identity','source_bundles_in_order','outputs_in_order',
        'source_revalidation_performed','complete_reconstruction_claimed','disposition'}
    if (not isinstance(record,dict) or set(record)!=fields
            or record.get('schema')!='aws_c0_runtime_control_reconstruction_source_attachment/v5'
            or record.get('read_plan_identity')!=read_plan_identity or record.get('phase')!=phase
            or record.get('validation_utc')!=validation_utc
            or record.get('sealed_context_identity')!=expected_sealed_context_identity
            or record.get('source_revalidation_performed') is not True
            ...
            or record.get('disposition')!='PARTIAL_SOURCE_BOUND_NOT_READY'
            or not isinstance(record.get('source_bundles_in_order'),list)
            or len(record['source_bundles_in_order'])!=8):
        raise Refusal('closed v5 source-bound partial attachment required')
    bundles=record['source_bundles_in_order']
    expected=build_runtime_control_reconstruction_source_attachment_v5(read_plan=read_plan,
        read_plan_identity=read_plan_identity,phase=phase,validation_utc=validation_utc,
        sealed_context=record['sealed_context'],sealed_context_identity=record['sealed_context_identity'],
        account_bundle=bundles[0],instance_profile_bundle=bundles[1],iam_bundle=bundles[2],
        bucket_controls_kms_bundle=bundles[3],vpc_bundle=bundles[4],service_quota_bundle=bundles[5],
        change_set_effects_bundle=bundles[6],artifact_version_set_bundle=bundles[7])
    if canonical_bytes(record)!=canonical_bytes(expected):
        raise Refusal('v5 source-bound attachment differs from rerun exact reconstruction')
    return record
...
def _validate_live_packet_v9(packet: dict[str, Any]) -> None:
    """Validate the truthful prospective predeployment carrier.

    V9 preserves the nine predecessor preimages and every action, permission, session,
    cost, science and software-drift field.  It directly embeds the eight
    controls currently eligible for source reconstruction before deployment.
    """
    if set(packet) != LIVE_PACKET_V9_FIELDS or packet.get("schema") != "aws_c0_live_packet/v9":
        raise Refusal("live-packet-v9 field closure failed")
    _common(packet, "aws_c0_live_packet/v9", root=False)
    plan=packet['runtime_control_read_plan'];plan_id=packet['runtime_control_read_plan_identity']
    validate_runtime_control_read_plan_v3(plan,plan_id)
    _identity(packet, "preparation_closure_identity", "aws_c0_preparation_closure/v8")
    _identity(packet, "launch_request_identity", "aws_c0_launch_request/v9")
    if (packet["packet_disposition"] != "AWS_C0_LIVE_PACKET_COMPLETE_UNAUTHORIZED"
            or packet["final_instance_state"] != "stopped"
            or packet["pre_live_object_count"] != FROZEN_PRELIVE_OBJECT_COUNT):
        raise Refusal("live packet disposition/count/state mismatch")
    if len(packet["pre_live_predecessor_object_receipts"]) != FROZEN_PRELIVE_PREDECESSOR_COUNT:
        raise Refusal("live packet predecessor receipt count mismatch")
    _identity(packet, 'deployment_inputs_identity', 'aws_c0_deployment_inputs/v1')
    _deployment_control_values(packet['runtime_control_preimages'],PREDEPLOYMENT_CONTROLS,
                               '1970-01-01T00:00:00Z',packet['observed_utc'])
    attachment=packet['runtime_control_reconstruction_source_attachment']
    attachment_id=packet['runtime_control_reconstruction_source_attachment_identity']
    expected_id=identity('aws_c0_runtime_control_reconstruction_source_attachment/v5',
                         digest(canonical_bytes(attachment)))
    if attachment_id!=expected_id:
        raise Refusal('live packet source attachment identity mismatch')
    validate_runtime_control_reconstruction_source_attachment_v5(attachment,read_plan=plan,
        read_plan_identity=plan_id,phase='PREDEPLOYMENT',validation_utc=attachment.get('validation_utc'),
        expected_sealed_context_identity=attachment.get('sealed_context_identity'))
    if (_utc(attachment['validation_utc'])>_utc(packet['observed_utc'])
            or [item.get('control') for item in attachment['outputs_in_order']]!=list(PREDEPLOYMENT_SOURCE_CONTROLS)):
        raise Refusal('live packet source attachment chronology/control order mismatch')
    change_output=attachment['outputs_in_order'][6];decoded=change_output['decoded_json']
    if (packet.get('change_set_observation_identity')!=change_output.get('identity')
            or packet.get('effect_api_set_identity')!=decoded.get('effect_api_set_identity')
            or packet.get('effect_resource_set_identity')!=decoded.get('effect_resource_set_identity')):
        raise Refusal('live packet change-set source projections differ from attachment')
    for identity_field, bytes_field, kind in (
        ("execution_session_policy_identity", "execution_session_policy_canonical_json_base64", "aws_iam_session_policy/v1"),
        ("execution_session_policy_ceiling_identity", "execution_session_policy_ceiling_canonical_json_base64", "aws_iam_policy_ceiling/v1"),
        ("execution_session_policy_subset_proof_identity", "execution_session_policy_subset_proof_canonical_json_base64", "aws_iam_policy_subset_proof/v1"),
...
def _validate_live_authorization_v9(auth: dict[str, Any]) -> None:
    if set(auth)!=LIVE_AUTH_V9_FIELDS or auth.get('schema')!='aws_c0_live_authorization/v9':
        raise Refusal('live-authorization-v9 field closure failed')
    _common(auth,'aws_c0_live_authorization/v9',root=False)
    _identity(auth,'live_packet_identity','aws_c0_live_packet/v9')
    _identity(auth,'runtime_control_reconstruction_source_attachment_identity',
...
def preflight(event: dict[str, Any]) -> dict[str, Any]:
    ...
    auth, _, auth_id = _fetch_receipt(event["live_authorization"], "aws_c0_live_authorization/v9", root=False)
    _validate_live_authorization_v9(auth)
    ...
    packet, _, packet_id = _fetch_receipt(auth["live_packet_object"], "aws_c0_live_packet/v9", root=False)
    _validate_live_packet_v9(packet)
    if auth.get("live_packet_identity") != identity("aws_c0_live_packet/v9", packet_id):
    ...
    launch, launch_receipt, launch_id = _fetch_receipt(packet["launch_request_object"], "aws_c0_launch_request/v9", root=True)
    _validate_launch_v9(launch)
    if packet.get("launch_request_identity") != identity("aws_c0_launch_request/v9", launch_id):
    ...
    return {
        ...
        "launch_request_identity": identity("aws_c0_launch_request/v9", launch_id),
        "launch_request_object": launch_receipt,
        "live_packet_identity": identity("aws_c0_live_packet/v9", packet_id),
        "live_packet_object": auth["live_packet_object"],
        "live_authorization_identity": identity("aws_c0_live_authorization/v9", auth_id),
...
def _resource_use(launch: dict[str, Any], seed_id: str, seed_receipt: dict[str, Any],
    ...
    rows = [{"dimension": name, "unit": UNITS[name], "observation_disposition": "SEALED_MAXIMUM_SUBSTITUTION",
             ...
             "maximum_substitution_source_identity": identity("aws_c0_launch_request/v9", launch_id)}
    ...
    return _control_record("aws_c0_resource_use_closure/v3", {
        ...
        "closure_seed_object": seed_receipt, "launch_request_identity": identity("aws_c0_launch_request/v9", launch_id),
...
def _semantic_roots(launch: dict[str, Any], launch_receipt: dict[str, Any], launch_id: str,
    ...
    roots.append((identity("aws_c0_launch_request/v9", launch_id), launch_receipt))
...
def _verify_prelive_objects(packet_receipt: dict[str, Any], auth_receipt: dict[str, Any],
    ...
    packet, _, packet_id = _fetch_receipt(packet_receipt, "aws_c0_live_packet/v9", root=False)
    auth, _, auth_id = _fetch_receipt(auth_receipt, "aws_c0_live_authorization/v9", root=False)
    _validate_live_packet_v9(packet); _validate_live_authorization_v9(auth)
    if auth["live_packet_identity"] != identity("aws_c0_live_packet/v9", packet_id):
    ...
    verified.extend([
        {"record_or_object_identity": identity("aws_c0_live_packet/v9", packet_id), "object": packet_receipt,
         "verification_disposition": "EXACT_VERSION_COMPLETE_BYTES_PASS"},
        {"record_or_object_identity": identity("aws_c0_live_authorization/v9", auth_id), "object": auth_receipt,
         "verification_disposition": "EXACT_VERSION_COMPLETE_BYTES_PASS"},
    ])
    return verified
...
def closure(event: dict[str, Any]) -> dict[str, Any]:
    ...
    seed, seed_receipt, seed_id = _exact_environment_record("CLOSURE_SEED", "aws_c0_closure_seed/v2", root=False)
    model, model_receipt, model_id = _exact_environment_record("COST_MODEL", "aws_c0_cost_model/v2", root=False)
    launch, launch_receipt, launch_id = _exact_environment_record("LAUNCH", "aws_c0_launch_request/v9", root=True)
    _validate_launch_v9(launch)
    _validate_cost_model(model, model_id, launch)
